...
# 载入图片
bg_sky = pygame.image.load('src/background.png')
# grass
bg_grass = pygame.image.load('src/grass.png')
bg_grass1 = pygame.image.load('src/grass2.png')
# tree
bg_tree = pygame.image.load('src/smalltree.png')
# birds
...
class Bird(pygame.sprite.Sprite):
    def __init__(self, imgs: list, x: float, y: float, speed_x=0, rotate=0, horizontal=False):
        super().__init__()
        self.imgs = imgs
        self.image = self.imgs[0]
        self.current = 0
        self.speed = speed_x
        self.drop_y = 0.15  # 掉落速度
        self.lift_y = 0.25  # 上升速度
        self.averageY = y  # 鸟的平均高度
        self.range = 30 + random.randint(10, 40)  # 飞行y区间
        self.drop = 0
        self.rect = self.imgs[0].get_rect()
        self.rect.x = x
        self.rect.y = y
        self.rotate = rotate  # 旋转速度
        self.degree = 0
        self.alive = True
        ...
        if self.alive:
            ...
            if not self.horizontal:
                if not self.birdUp:
                    ...
                    self.drop = -self.lift_y
                    self.rect.y += self.drop
                    self.degree = 30
                self.image = pygame.transform.rotate(self.image, self.degree)
            else:
                # 水平飞行  sin 函数路径
                self.rect.y = self.averageY + self.range * \
                    math.sin(math.pi / 100 * self.rect.x)
...
def InitNewBird():
    ...
    if flappyBird is None:
        ...
        flappyBird = Bird(kindOfBird[birdimg], 0, y, speed, 0, horizontal=True)
...
title = Button(title_pic, 512-89., 190., 0.)
ready = Button(ready_pic, 512-98., 260., 0.)
over = Button(over_pic, 512-98., 240., 0.)

score = Score(screen, 1000, 480)

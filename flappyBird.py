...
# 载入图片
bg_sky = pygame.image.load('src/sky.png')
# grass
...
def InitNewBird():
    ...
    if flappyBird is None:  
        ...
        flappyBird = Bird(kindOfBird[birdimg], 200, 100, 0, 1)
    ...
    if not gameStart:
        ...
        if not gameOver:
            for _pipe in pipes.sprites()[:4]:
                ...
                if pygame.sprite.collide_rect_ratio(0.93)(flappyBird, _pipe):
                    gameOver = True
                    break
                # 计分
                    ...
                    if index == 0:
                        ...
                        if not gameStart:
                            ...
                            for i in range(5):
                                up_y = 280 + 100*random.random()
                                _up, _down = PipePair(
                                    500+i*200, up_y=up_y, gap=pipe_gap, speed=pipe_speed)
                                pipes.add(_up)
                                pipes.add(_down)
